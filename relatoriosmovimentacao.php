...
<body>
    <div id="base">
        ...
        <?php
            ...
            $sql = "SELECT * FROM `movimentacao` as m join cliente as c on (c.id=m.cliente_id)"; 
            $sql = $con->prepare($sql);
            $sql -> execute();
            ...
            $sql2 = "  SELECT count(categoria) as c FROM `conteiner`  WHERE categoria='Importação' "; 
            $sql2 = $con->prepare($sql2);
            $sql2 -> execute();
            
            while($linha2 = $sql2->fetch(PDO::FETCH_ASSOC)){ 
               
                echo "<br>| Total de Importações ".$linha2['c']."                           |";
              
            }
            $sql3 = "  SELECT count(categoria) as c FROM `conteiner`  WHERE categoria='Exportação' "; 
            $sql3 = $con->prepare($sql3);
            $sql3 -> execute();
            
            while($linha3 = $sql3->fetch(PDO::FETCH_ASSOC)){ 
             
                echo "<br>| Total de Exportações ".$linha3['c']."                           |";
